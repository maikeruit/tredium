                    ...
                    @foreach($tags as $tag)
                        <a href="{{ route('articles.index', ['tag' => $tag->slug]) }}" 
                           ...
                           {{ request('tag') == $tag->slug 
                            ...
                            {{ $tag->name }} <span class="text-{{ request('tag') == $tag->slug ? 'blue-200' : 'gray-500' }}">({{ $tag->articles_count }})</span>
                        </a>
                    @endforeach
                    
                    @if(request()->has('tag'))
                        <div class="mt-4">
                            <a href="{{ route('articles.index') }}" class="text-blue-500 hover:text-blue-700 text-sm">
                                ← Сбросить фильтр
                            </a>
                        </div>
                    @endif
